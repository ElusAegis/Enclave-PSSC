...
class EthOracleService(apiUrl: String,
    ...
    init {
        ...
        if (!nodeSynced) {
            ...
            logger.info("Successfully connected to node running $web3ClientVersion")
        }
    }
    ...
    fun doEthDataCall(
        ...
        val response = web3.ethCall(callTransaction) { "latest" }.send()
        if (response.isReverted) {
            logger.error("Eth call reverted: ${response.revertReason}")
            throw IllegalStateException("Eth call reverted: ${response.error}")
        }
